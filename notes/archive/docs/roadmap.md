...
- [x] **1.3** Seed list edited — one candidate added (`dec-0001`), seven cut. See
      [ADR-009](decisions/ADR-009-phase1-twelve.md).
- [x] **1.4** Cut to 12, mix corrected to 4 decisions / 5 workflows / 3 skills-configs —
      [ADR-009](decisions/ADR-009-phase1-twelve.md).
- [ ] **1.5** Write all 12 to the [artifact spec](artifact-spec.md).
      **7 of 12 drafted** — all 4 decisions and the 3 wedge workflows.
      Remaining: `wf-0004`, `wf-0005`, `skill-0001`, `skill-0002`, `cfg-0001`.
      ⚠️ Per ADR-009, **ask the Gate M3 question before writing `wf-0005`** — if stakeholder
      reporting beats the week-one ramp, the mix is wrong.
- [ ] **1.6** Run **Gate A** on the drafted 7: fresh session, two models, worked example
      committed. **Nothing leaves `draft` until this runs — this is founder work, not desk work.**
      First action: pin `FIXTURE_REPO` / `FIXTURE_SHA` in [catalog/evals/README.md](../catalog/evals/README.md).
- [ ] **1.7** Run **Gate B**: measure baseline, measure with-artifact, record the real number.
      Every `outcome:` field is currently empty and must stay empty until measured.
- [x] **1.8** Evals written for the 7 drafted artifacts, plus the fixture-repo rules.
      Remaining 5 evals land with their artifacts.
...
measured outcome. **Currently 0 verified, 7 drafted.** The gap between those two numbers is the
whole of Phase 1 — drafting is the cheap half.
